...
def getbatch(data,labels,i,batch_size):
	...
	xd,xl,td,ty,tl = prep_data(data[i:i+batch_size],labels[i:i+batch_size])
...
# devf,devt = get_validation_data()
# xd,xl,tx,ty,tl = getbatch(devf[:10],devt[:10],0,5)
# k,v,l = modelEncoder(xd,xl)
# ps,att = modelDecoder(tx,k,v,l)
...
def train():
	...
	modelDecoder.train()
	total_loss = 0
	#r_sampler = np.random.choice(training_data.shape[0],training_data.shape[0],replace=False)
	for i in range(0,training_data.shape[0],batch_size):
		...
		if i % log_step == 0 and i > 0:
			curr_loss = total_loss[0]/ batch_size / log_step
			print(curr_loss)
			total_loss = 0
			#translate(ps.data,ty.data)


save_fileenc = "hahaenc.pt"
save_filedec = "hahadec.pt"
epoch = 100
best_loss = validate()
